...
(
    ...
    `step_type`         TINYINT      NOT NULL DEFAULT 1 COMMENT '步骤类型 1-单接口 2-组合组件 3-生成变量',
    `component_id`      BIGINT       DEFAULT NULL COMMENT '组合组件ID(step_type=2时引用)',
    `generator_id`      BIGINT       DEFAULT NULL COMMENT '数据生成器ID(step_type=3时引用)',
    `variable_name`     VARCHAR(100) DEFAULT NULL COMMENT '生成值写入的变量名(step_type=3时必填)',
    `regen_each_run`    TINYINT      NOT NULL DEFAULT 1 COMMENT '每轮是否重新生成 0-否 1-是(step_type=3)',
